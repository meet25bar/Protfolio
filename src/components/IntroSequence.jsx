...
export default function IntroSequence({ onComplete }) {
  ...
  useEffect(() => {
    // 0. Welcome greeting
    // 1. TV line appears
    const t1 = setTimeout(() => setStage('turn-on'), 1800)
    // 2. Line expands horizontally
    const t2 = setTimeout(() => setStage('expand'), 2100)
    // 3. Full static and name
    const t3 = setTimeout(() => setStage('static'), 2400)
    // 4. Turn off line
    const t4 = setTimeout(() => setStage('turn-off'), 5200)
    // 5. Disappear
    const t5 = setTimeout(() => onComplete(), 5700)
      ...
      >
        ...
        <AnimatePresence>
          {stage === 'welcome' && (
            ...
            >
              <motion.p
                className="font-mono text-xs md:text-sm tracking-[0.4em] uppercase mb-4"
                style={{ color: 'rgba(99,179,237,0.6)' }}
              ...
              <motion.h2
                className="font-syne font-bold text-3xl md:text-5xl text-text-primary tracking-wide"
              ...
              <motion.div
                ...
                style={{ background: 'linear-gradient(90deg, transparent, rgb(99,179,237), transparent)' }}
        ...
        <AnimatePresence>
          {showTV && (
            <motion.div
              key="tv-chassis"
              className="relative z-10 w-[95vw] max-w-3xl aspect-[4/3] md:aspect-[1.2] bg-[#1C212D] rounded-[2rem] md:rounded-[3.5rem] p-3 md:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.9),inset_0_4px_10px_rgb(var(--text-primary) / 0.15)] border-b-[12px] border-r-4 border-[#0F1117] border-t-2 border-l-2 border-[#2A3142] flex flex-col items-center justify-center"
            ...
            >
              ...
              <div className="relative w-full h-full bg-[#0F1117] rounded-[1.2rem] md:rounded-[2.5rem] p-2 md:p-5 shadow-[inset_0_0_20px_rgba(0,0,0,1)] border border-[#161A23]">
                ...
                >
                  ...
                  >
                    <h1 
                      className="font-syne font-bold text-4xl md:text-6xl text-text-primary tracking-widest uppercase relative"
                    >
                      <span className="relative z-10" style={{ textShadow: '0 0 20px rgb(var(--text-primary) / 0.4)' }}>
                        Meet Barot
                      </span>
                      {/* Glitch RGB layers */}
