...
function useDockMagnification(mouseX, ref, standardSize = 44, magnifiedSize = 64, isMobile = false) {
...
function DockIcon({ mouseX, icon: Icon, label, isActive, isDark, onClick, glowColor }) {
  ...
  const magnifiedSize = isMobile ? standardSize : 62
  ...
  return (
    <div className={`fixed ${isMobile ? 'bottom-3' : 'bottom-6 md:bottom-8'} left-1/2 -translate-x-1/2 z-50`}>
      ...
      <motion.nav
        ...
        className={`flex items-end ${isMobile ? 'gap-1.5 px-2.5 py-2.5 rounded-[20px]' : 'gap-2 px-3 py-2.5 rounded-[22px]'} relative overflow-visible transition-all duration-300`}
        style={{
          boxShadow: isDark
